...
function buildPageImageUrl(slug: string, src: string): string {
  ...
  const encodedPath = pathPart
    ...
    .join('/')
  return `${defaultApiBaseUrl()}/pages/${encodeURIComponent(slug)}/images/${encodedPath}${suffix}`
}
...
async function fetchAndRenderMarkdown(slug: string) {
  ...
  try {
    const resp = await fetch(`${defaultApiBaseUrl()}/pages/${encodeURIComponent(slug)}`, {
