...
func (_q *UserSubscriptionQuery) QuerySubscriptionResetCardUsages() *SubscriptionResetCardUsageQuery {
...
func (_q *UserSubscriptionQuery) WithSubscriptionResetCardUsages(opts ...func(*SubscriptionResetCardUsageQuery)) *UserSubscriptionQuery {
	query := (&SubscriptionResetCardUsageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserSubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserSubscription, error) {
	...
	if query := _q.withSubscriptionResetCardUsages; query != nil {
		if err := _q.loadSubscriptionResetCardUsages(ctx, query, nodes,
			func(n *UserSubscription) { n.Edges.SubscriptionResetCardUsages = []*SubscriptionResetCardUsage{} },
			func(n *UserSubscription, e *SubscriptionResetCardUsage) {
...
func (_q *UserSubscriptionQuery) loadSubscriptionResetCardUsages(ctx context.Context, query *SubscriptionResetCardUsageQuery, nodes []*UserSubscription, init func(*UserSubscription), assign func(*UserSubscription, *SubscriptionResetCardUsage)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*UserSubscription)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(subscriptionresetcardusage.FieldSubscriptionID)
	}
	query.Where(predicate.SubscriptionResetCardUsage(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(usersubscription.SubscriptionResetCardUsagesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
