...
func TestAccountHandlerGetAvailableModels_GeminiOAuthDefaultsToGeminiModels(t *testing.T) {
	svc := &availableModelsAdminService{
		stubAdminService: newStubAdminService(),
		account: service.Account{
			ID:       46,
	...
	for _, model := range resp.Data {
		...
		require.NotContains(t, strings.ToLower(model.ID), "gpt")
	}
...
func TestAccountHandlerSyncUpstreamModels_OpenAIOAuthUnsupported(t *testing.T) {
	svc := &availableModelsAdminService{
		stubAdminService: newStubAdminService(),
		account: service.Account{
			ID:       46,
	...
	require.Equal(t, http.StatusBadRequest, rec.Code)
	require.Contains(t, rec.Body.String(), "OpenAI 认证登录账号暂不支持同步上游模型")
