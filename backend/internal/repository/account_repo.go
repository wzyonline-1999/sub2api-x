...
// ListStableQuotaLoadByGroupIDs returns the privacy-safe projection used for
// group quota-window aggregation. Unlike ListSchedulableCapacityByGroupIDs,
// this query deliberately retains accounts in temporary, overload, and rate
// limit cooldown windows so a fully consumed account cannot disappear from
// the aggregate. Only stable operator state and automatic expiry are filtered.
func (r *accountRepository) ListStableQuotaLoadByGroupIDs(ctx context.Context, groupIDs []int64) ([]service.GroupAccountQuotaLoadRow, error) {
	groupIDs = uniquePositiveInt64s(groupIDs)
	if len(groupIDs) == 0 {
		return []service.GroupAccountQuotaLoadRow{}, nil
	}
	now := time.Now()
	if r.sql == nil {
		rows := make([]service.GroupAccountQuotaLoadRow, 0)
		for _, groupID := range groupIDs {
			accounts, err := r.ListByGroup(ctx, groupID)
			if err != nil {
				return nil, err
			}
			for i := range accounts {
				...
				rows = append(rows, service.GroupAccountQuotaLoadRow{
					GroupID:          groupID,
	...
	out := make([]service.GroupAccountQuotaLoadRow, 0)
	for rows.Next() {
		var row service.GroupAccountQuotaLoadRow
		var extraRaw string
		if err := rows.Scan(
			&row.GroupID,
			&row.AccountID,
