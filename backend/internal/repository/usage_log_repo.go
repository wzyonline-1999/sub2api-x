...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		sessionIDSource       sql.NullString
		sessionHash           sql.NullString
		sessionExplicit       sql.NullBool
		createdAt             time.Time
	)
	...
	if sessionIDSource.Valid {
		log.SessionIDSource = &sessionIDSource.String
	}
	if sessionHash.Valid {
		log.SessionHash = &sessionHash.String
	}
	if sessionExplicit.Valid {
		log.SessionExplicit = &sessionExplicit.Bool
	}
