...
func TestUsageLogRepositoryCreateSyncRequestTypeAndLegacyFields(t *testing.T) {
	...
	mock.ExpectQuery("INSERT INTO usage_logs").
		WithArgs(
			...
			sqlmock.AnyArg(), // session_id_source
			sqlmock.AnyArg(), // session_hash
			sqlmock.AnyArg(), // session_explicit
			createdAt,
		).
...
func TestUsageLogRepositoryEndpointStatsApplyUpstreamModelMismatchFilter(t *testing.T) {
	db, mock := newSQLMock(t)
	repo := &usageLogRepository{sql: db}
	...
	mock.ExpectQuery("(?s)TRIM\\(inbound_endpoint\\).*upstream_model_mismatch IS TRUE.*GROUP BY endpoint").
		WithArgs(start, end).
		WillReturnRows(emptyRows())
	_, err := repo.getEndpointStatsByColumnWithFilters(context.Background(), "inbound_endpoint", start, end, 0, 0, 0, 0, "", "", nil, nil, nil, "", &mismatch)
	require.NoError(t, err)

	mock.ExpectQuery("(?s)TRIM\\(upstream_endpoint\\).*upstream_model_mismatch IS TRUE.*GROUP BY endpoint").
		WithArgs(start, end).
		WillReturnRows(emptyRows())
	_, err = repo.getEndpointStatsByColumnWithFilters(context.Background(), "upstream_endpoint", start, end, 0, 0, 0, 0, "", "", nil, nil, nil, "", &mismatch)
	require.NoError(t, err)

	mock.ExpectQuery("(?s)SELECT\\s+CONCAT\\(.*upstream_model_mismatch IS TRUE.*GROUP BY endpoint").
		WithArgs(start, end).
		WillReturnRows(emptyRows())
	_, err = repo.getEndpointPathStatsWithFilters(context.Background(), start, end, 0, 0, 0, 0, "", "", nil, nil, nil, "", &mismatch)
	require.NoError(t, err)
...
func TestUsageLogRepositoryGetUsageRankingKeepsCurrentUserSeparateWhenOutsideLimit(t *testing.T) {
	...
	require.Equal(t, &usagestats.UsageRankingItem{
		Rank: 18, UserID: 42, DisplayName: "me", Requests: 2, TotalTokens: 1200000, ActualCost: 9.71,
	}, got.CurrentUser)
	require.Equal(t, &usagestats.UsageRankingTarget{
...
func TestUsageLogRepositoryGetUsageRankingTopUserHasNoTarget(t *testing.T) {
	db, mock := newSQLMock(t)
	repo := &usageLogRepository{sql: db}
	...
	require.NoError(t, err)
	require.Equal(t, &usagestats.UsageRankingItem{
		Rank: 1, UserID: 42, DisplayName: "top-user", Requests: 12, TotalTokens: 128600000, ActualCost: 1842.62,
	}, got.CurrentUser)
	require.Equal(t, &usagestats.UsageRankingTarget{
...
func TestUsageLogRepositoryGetUsageRankingUsesNearestHigherRankWhenRanksTie(t *testing.T) {
	db, mock := newSQLMock(t)
	repo := &usageLogRepository{sql: db}
	...
	require.NoError(t, err)
	require.Equal(t, &usagestats.UsageRankingItem{
		Rank: 3, UserID: 42, DisplayName: "me", Requests: 7, TotalTokens: 800, ActualCost: 8.00,
	}, got.CurrentUser)
	require.Equal(t, &usagestats.UsageRankingTarget{
		TargetType:        usagestats.UsageRankingTargetPrevious,
		TargetRank:        int64Ptr(1),
...
func TestUsageLogRepositoryGetUsageRankingUsesNearestThresholdWhenRanksSkip(t *testing.T) {
	db, mock := newSQLMock(t)
	repo := &usageLogRepository{sql: db}
	...
	require.NoError(t, err)
	require.Equal(t, &usagestats.UsageRankingItem{
		Rank: 11, UserID: 42, DisplayName: "me", Requests: 7, TotalTokens: 250, ActualCost: 2.50,
	}, got.CurrentUser)
	require.Equal(t, &usagestats.UsageRankingTarget{
		TargetType:        usagestats.UsageRankingTargetThreshold,
...
func TestScanUsageLogRequestTypeAndLegacyFallback(t *testing.T) {
	...
	t.Run("request_type_ws_v2_overrides_legacy", func(t *testing.T) {
		...
		log, err := scanUsageLog(usageLogScannerStub{values: []any{
			...
			sql.NullString{},  // session_id_source
			sql.NullString{},  // session_hash
			sql.NullBool{},    // session_explicit
			now,
		}})
		require.NoError(t, err)
