...
func explicitOpenAIHeaderSessionIDWithSource(c *gin.Context) (string, string) {
	if c == nil {
		return "", ""
	}
...
func explicitOpenAIHeaderSessionID(c *gin.Context) string {
	sessionID, _ := explicitOpenAIHeaderSessionIDWithSource(c)
	return sessionID
}
...
// explicitOpenAISessionIDWithSource extends the common OpenAI session signals
// with Grok-only conversation signals. previous_response_id is used only as a
// last-resort sticky seed for Grok Responses chains; non-Grok requests retain
// the existing header/prompt-cache behavior.
func explicitOpenAISessionIDWithSource(c *gin.Context, body []byte) (string, string) {
	if c == nil {
		return "", ""
	}

	sessionID, source := explicitOpenAIHeaderSessionIDWithSource(c)
	if sessionID != "" {
	...
	if isGrokRequestContext(c) && len(body) > 0 {
		if sessionID = grokPreviousResponseSessionSeed(body); sessionID != "" {
			return sessionID, "previous_response_id"
		}
	}
...
func resolveOpenAISessionMetadata(c *gin.Context, body []byte, includeContentFallback bool, fallbackSeed string, fallbackSource string) OpenAISessionMetadata {
	...
	if sessionID, source := explicitOpenAISessionIDWithSource(c, body); sessionID != "" {
		...
		// never persist the raw value in usage_logs.session_id. The Grok
		// previous_response_id seed is namespaced routing material rather than a
		// client session ID, so it is likewise not persisted raw.
		rawSessionID := sessionID
		if source == "prompt_cache_key" || source == "previous_response_id" {
			rawSessionID = ""
		}
		routingSeed := sessionID
		if includeContentFallback {
			// Stateless explicit-only endpoints (notably /v1/images) retain the
			// upstream GenerateExplicitSessionHash behavior and do not add model
			// scoping to their client-provided key.
			routingSeed = openAISessionRoutingSeed(c, body, sessionID)
		}
		return openAISessionMetadataFromSeed(c, routingSeed, source, rawSessionID, true)
	}

	if includeContentFallback && len(body) > 0 {
		if seed := deriveOpenAIContentSessionSeed(body); seed != "" {
			return openAISessionMetadataFromSeed(c, openAISessionRoutingSeed(c, body, seed), "content_fallback", "", false)
		}
	}
...
// openAISessionRoutingSeed keeps metadata and scheduler affinity on the same
// hash while adopting Grok's model-scoped sticky routing. A missing model is
// left unchanged: such a request is invalid for normal HTTP routing, and WS
// fallback seeds intentionally retain their legacy cross-turn identity.
func openAISessionRoutingSeed(c *gin.Context, body []byte, seed string) string {
	if !isGrokRequestContext(c) || strings.TrimSpace(gjson.GetBytes(body, "model").String()) == "" {
		return seed
	}
	return grokStickyAffinitySeed(seed, body)
}
...
func (s *OpenAIGatewayService) GenerateExplicitSessionHash(c *gin.Context, body []byte) string {
	return s.ResolveExplicitOpenAISessionMetadata(c, body).SessionHash
}
...
//  7. Body:   previous_response_id (Grok groups only, resp_* values only)
