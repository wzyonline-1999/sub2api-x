...
type revisionCASCacheStub struct {
	*billingCacheMissStub

	mu        sync.Mutex
	revision  int64
	stored    *SubscriptionCacheData
	setCalls  int
	mutateCAS bool
}

func (s *revisionCASCacheStub) GetSubscriptionCache(context.Context, int64, int64) (*SubscriptionCacheData, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.stored == nil {
		return nil, errors.New("cache miss")
...
func (s *revisionCASCacheStub) GetSubscriptionCacheRevision(context.Context, int64, int64) (int64, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.revision, nil
}

func (s *revisionCASCacheStub) SetSubscriptionCacheIfRevision(_ context.Context, _, _, expectedRevision int64, data *SubscriptionCacheData) (bool, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.setCalls++
	if s.mutateCAS {
...
func (s *revisionCASCacheStub) snapshot() (*SubscriptionCacheData, int) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.stored == nil {
		return nil, s.setCalls
	}
	cp := *s.stored
	return &cp, s.setCalls
}

type subscriptionSnapshotSequenceRepo struct {
	userSubRepoNoop
...
func (r *subscriptionSnapshotSequenceRepo) GetActiveByUserIDAndGroupID(context.Context, int64, int64) (*UserSubscription, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	index := r.calls
...
func (r *subscriptionSnapshotSequenceRepo) callCount() int {
	r.mu.Lock()
	defer r.mu.Unlock()
	return r.calls
}

func TestBillingCacheServiceGetSubscriptionStatus_RetriesRevisionConflict(t *testing.T) {
	now := time.Now()
	cache := &revisionCASCacheStub{
		billingCacheMissStub: &billingCacheMissStub{},
		mutateCAS:            true,
	}
	repo := &subscriptionSnapshotSequenceRepo{
		snapshots: []UserSubscription{
			{
				UserID:               1,
				GroupID:              2,
	...
	require.Equal(t, 2, repo.callCount(), "a mutation between the DB read and CAS must force a new DB snapshot")

	stored, setCalls := cache.snapshot()
	require.NotNil(t, stored)
	require.Equal(t, float64(3), stored.DailyUsage)
