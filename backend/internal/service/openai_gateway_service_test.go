...
func TestOpenAIGatewayService_ResolveOpenAISessionMetadata_ExplicitSources(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "OpenCode affinity header is preserved",
			headers:     map[string]string{openCodeSessionAffinityHeader: "opencode-affinity"},
			wantID:      "opencode-affinity",
			wantSource:  "header_x_session_affinity",
			wantHashFor: "opencode-affinity",
		},
		{
			name:        "OpenCode session ID header is preserved",
			headers:     map[string]string{openCodeSessionIDHeader: "opencode-session-id"},
			wantID:      "opencode-session-id",
			wantSource:  "header_x_session_id",
			wantHashFor: "opencode-session-id",
		},
		{
			name:        "OpenCode native session header is preserved",
			headers:     map[string]string{openCodeNativeSessionHeader: "opencode-native-session"},
			wantID:      "opencode-native-session",
			wantSource:  "header_x_opencode_session",
			wantHashFor: "opencode-native-session",
		},
		{
			name:        "CodeBuddy conversation header is preserved",
			headers:     map[string]string{codeBuddyConversationHeader: "codebuddy-conversation"},
			wantID:      "codebuddy-conversation",
			wantSource:  "header_x_conversation_id",
...
func TestOpenAIGatewayService_ResolveOpenAISessionMetadata_CompositeGrokConversationHeader(t *testing.T) {
	gin.SetMode(gin.TestMode)
	svc := &OpenAIGatewayService{}
	bodyWithKey := []byte(`{"prompt_cache_key":"body-cache-key"}`)

	newContext := func(targetPlatform string) *gin.Context {
		rec := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(rec)
		req := httptest.NewRequest(http.MethodPost, "/openai/v1/responses", nil)
		req = req.WithContext(WithResolvedTargetPlatform(req.Context(), targetPlatform))
		c.Request = req
		c.Request.Header.Set(grokConversationIDHeader, "composite-grok-session")
		c.Set("api_key", &APIKey{ID: 1, Group: &Group{Platform: PlatformComposite}})
		return c
	}

	t.Run("composite route targeting Grok accepts the native conversation header", func(t *testing.T) {
		got := svc.ResolveOpenAISessionMetadata(newContext(PlatformGrok), bodyWithKey)

		require.Equal(t, "composite-grok-session", got.SessionID)
		require.Equal(t, "header_x_grok_conv_id", got.SessionIDSource)
		require.True(t, got.SessionExplicit)
	})
